...
public class DiscoveryParchment implements Parchment {
    ...
    public Recipe<? extends Inventory> getRecipe(RecipeManager manager) {
        return (finishedNewGatherQuest && !placedAnalyzer) ? (Recipe<? extends Inventory>) manager.get(new Identifier(ArcaneMagic.DOMAIN, "analyzer")).orElse(null) :
                (analyzedStick && !craftedScepter) ? (Recipe<? extends Inventory>) manager.get(new Identifier(ArcaneMagic.DOMAIN, "golden_scepter")).orElse(null) :
                        (craftedScepter) ? (Recipe<? extends Inventory>) manager.get(new Identifier(ArcaneMagic.DOMAIN, "notebook")).orElse(null) : null;
    }
